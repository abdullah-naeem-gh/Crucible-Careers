...
function ProfileSections({ candidate, job }: { candidate: CandidateProfile; job: EmployerJob }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {candidate.experience && candidate.experience.length > 0 && <TimelineSection title="Work Experience" items={candidate.experience.map((item) => ({ id: item.id, title: item.role, meta: item.company, range: `${item.startDate} - ${item.current ? "Present" : item.endDate}`, description: item.description, verified: item.verificationStatus === "verified" }))} />}
