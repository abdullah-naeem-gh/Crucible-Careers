...
function ProfileSections({ candidate, job }: { candidate: CandidateProfile; job: EmployerJob }) {
  // Anything not already shown in a dedicated section above (Quick Facts,
  // Skills, Resume, Links, Bio) — so a field never silently disappears just
  // because it isn't tagged "custom" but also has no dedicated display yet.
  const extraAnswers = (candidate.customAnswers ?? []).filter((answer) => !hasDedicatedDisplay(answer.semanticType));
  return (
    <div className="space-y-6">
      {candidate.bio && <section><h3 className="mb-2 text-xs font-semibold uppercase tracking-[0.14em] text-white/40">Candidate Bio</h3><p className="text-sm leading-relaxed text-white/60">{candidate.bio}</p></section>}
      {candidate.coverLetter && <section><h3 className="mb-2 text-xs font-semibold uppercase tracking-[0.14em] text-white/40">Cover Letter</h3><p className="whitespace-pre-line text-sm leading-relaxed text-white/60">{candidate.coverLetter}</p></section>}
      ...
      {extraAnswers.length > 0 && <section className="border-t border-white/[0.07] pt-5"><h3 className="mb-3 text-xs font-semibold uppercase tracking-[0.14em] text-white/40">Questionnaire Answers</h3><div className="grid grid-cols-1 gap-3 sm:grid-cols-2">{extraAnswers.map((answer) => <div key={answer.fieldId} className={`${insetSurface} p-4 sm:col-span-2`}><div className="mb-1 text-[10px] font-semibold uppercase tracking-wider text-white/35">{answer.label}</div><div className="whitespace-pre-line text-sm font-medium text-white/80">{Array.isArray(answer.value) ? answer.value.join(", ") : String(answer.value || "-")}</div></div>)}</div></section>}
      {candidate.experience && candidate.experience.length > 0 && <TimelineSection title="Work Experience" items={candidate.experience.map((item) => ({ id: item.id, title: item.role, meta: item.company, range: `${formatProfileMonthYear(item.startDate)} - ${item.current ? "Present" : formatProfileMonthYear(item.endDate)}`, description: item.description, verified: item.verificationStatus === "verified" }))} />}
      {candidate.educationList && candidate.educationList.length > 0 && <TimelineSection title="Education" items={candidate.educationList.map((item) => ({ id: item.id, title: item.degree, meta: item.school, range: `${formatProfileMonthYear(item.startYear ?? "")} - ${formatProfileMonthYear(item.endYear ?? "")}`, description: item.field }))} />}
...
function TimelineSection({ title, items }: { title: string; items: Array<{ id: string; title: string; meta: string; range: string; description?: string; verified?: boolean }> }) {
  return <section className="border-t border-white/[0.07] pt-5"><h3 className="mb-3 text-xs font-semibold uppercase tracking-[0.14em] text-white/40">{title}</h3><div className="space-y-3">{items.map((item) => <div key={item.id} className="rounded-xl border border-white/[0.04] bg-white/[0.01] p-4"><div className="flex flex-wrap items-center justify-between gap-1"><h4 className="text-[13px] font-bold text-white/90">{item.title}</h4><span className="rounded bg-orange-500/10 px-2 py-0.5 text-[10px] font-semibold text-[#FF914D]">{item.range}</span></div><div className="mt-0.5 flex items-center gap-1.5"><span className="text-xs font-medium text-white/50">{item.meta}</span>{item.verified && <span className="inline-flex items-center gap-1 rounded-full border border-emerald-500/30 bg-emerald-500/10 px-1.5 py-0.5 text-[9px] font-semibold text-emerald-300"><IconCheck size={10} /> Verified</span>}</div>{item.description && <p className="mt-2 text-xs leading-relaxed text-white/40">{item.description}</p>}</div>)}</div></section>;
}
