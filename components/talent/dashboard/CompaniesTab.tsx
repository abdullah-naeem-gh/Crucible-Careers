...
export default function CompaniesTab() {
  ...
  const selectedCompany = DEMO_COMPANIES.find(c => c.id === selectedCompanyId) ?? filteredCompanies[0] ?? null
      ...
      <section className="bg-white/70 backdrop-blur-sm border border-gray-100 rounded-[24px] shadow-[12px_12px_30px_rgba(0,0,0,0.035),-6px_-6px_18px_rgba(255,255,255,0.5)] flex flex-col overflow-hidden lg:col-span-5">
        <div className="border-b border-gray-100 px-5 py-5 flex items-end justify-between">
          ...
          <div className="text-right">
            <div className="text-xl font-bold text-gray-900">{DEMO_COMPANIES.length}</div>
          ...
          {filteredCompanies.map(c => (
            ...
            >
              <div className="flex items-center gap-4">
                <div className={`w-11 h-11 rounded-xl bg-gradient-to-br ${c.color} flex items-center justify-center shadow-inner shrink-0`}>
      ...
      <section className="bg-white/70 backdrop-blur-sm border border-gray-100 rounded-[24px] shadow-[12px_12px_30px_rgba(0,0,0,0.035),-6px_-6px_18px_rgba(255,255,255,0.5)] overflow-auto p-6 lg:col-span-4">
        {!selectedCompany ? (
          ...
          <motion.div key={selectedCompany.id} initial={{ opacity: 0, x: 8 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
            <div className="flex items-center gap-4">
              <div className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${selectedCompany.color} flex items-center justify-center shadow-inner shrink-0`}>
                <span className="text-xl font-bold text-white">{selectedCompany.logo}</span>
              </div>
              <div>
                <h2 className="text-lg font-bold text-gray-900 leading-tight">{selectedCompany.name}</h2>
                <div className="mt-1 text-xs text-gray-500">{selectedCompany.location}</div>
                <a href={selectedCompany.website} target="_blank" rel="noopener noreferrer" className="text-[10px] text-[#FF6B00] font-semibold hover:underline mt-1 block">
                  Visit Website ↗
                </a>
              </div>
            </div>
            ...
            <div className="border-t border-gray-100 pt-5">
              ...
              <p className="text-xs text-gray-600 leading-relaxed">{selectedCompany.about}</p>
            </div>
