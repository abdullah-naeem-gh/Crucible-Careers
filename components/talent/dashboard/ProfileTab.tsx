...
function ProfilePreview({ profile }: { profile: TalentProfile }) {
  ...
  return (
    <motion.div
      key={profile.id + (profile as any).updatedAt + profile.firstName + profile.lastName + profile.headline}
        ...
        <div className="flex items-start gap-4">
          <div className="relative h-16 w-16 shrink-0 overflow-hidden rounded-2xl border border-gray-200 bg-gradient-to-br from-orange-100 to-white dark:border-white/[0.08] dark:from-[#FF6B00]/20 dark:to-[#FF914D]/10">
            {profile.photoUrl ? (
              ...
              <span className="absolute inset-0 grid place-items-center text-lg font-bold text-[#FF6B00]">{initials(profile.firstName, profile.lastName)}</span>
            )}
          </div>
            ...
            {visibleProjects.map((item) => (
              <div key={item.id} className="rounded-xl border border-gray-200 bg-gray-50/70 p-3 dark:border-white/[0.06] dark:bg-white/[0.025]">
                <div className="flex gap-3">
                  {item.imageUrl && (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={item.imageUrl} alt="Project" className="h-14 w-16 shrink-0 rounded-lg object-cover" />
                  )}
                  <div className="min-w-0 flex-1">
      ...
      {(profile.linkedin || profile.github || profile.portfolio || profile.introVideoUrl || profile.resumeFilename) && (
        <PreviewSection title="Links and media">
          <div className="grid grid-cols-1 gap-2 text-xs sm:grid-cols-2">
            ...
            {profile.resumeUrl && <a href={normalizeUrl(profile.resumeUrl)} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-gray-600 dark:text-white/45 hover:text-orange-500"><IconFileText size={15} /> {profile.resumeFilename || 'View Resume'}</a>}
          </div>
        </PreviewSection>
      )}
    </motion.div>
...
export default function ProfileTab({ profile, onProfileChange }: ProfileTabProps) {
  ...
  const createProfile = (newProfile: TalentProfile) => {
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-3">
                <button
                  type="button"
                  onClick={() => saveProfile()}
                  className="flex cursor-pointer items-center gap-1.5 rounded-xl bg-orange-500 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-orange-600 dark:bg-[#FF6B00]"
                >
